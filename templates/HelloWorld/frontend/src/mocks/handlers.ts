...
// In-memory store — state persists only for the dev session.
let notesStore: MockNote[] = [
  {
    id: "note-1",
    title: "Sample note",
    body: "Edit src/mocks/handlers.ts to change this.",
...
const handlers: Record<string, MockHandler> = {
  ...
  echo: (payload) => ({
    ...
    context: {
      ...
      requestor: { type: "user", userId: "mock-user-id", email: "dev@localhost", name: "Local Developer", applicationPermissions: ["read", "write", "export", "approve"] },
  ...
  listDocuments: () => ({ notes: notesStore, cursor: undefined }),

  updateDocument: (payload: unknown) => {
    const { id, title, body } = (payload as { id?: string; title?: string; body?: string }) || {};
    const index = notesStore.findIndex((n) => n.id === id);
    if (index === -1) throw new Error("Note not found");
    notesStore[index] = {
      ...notesStore[index],
      title: title?.trim() || notesStore[index].title,
      body: body?.trim() ?? notesStore[index].body,
      updatedAt: new Date().toISOString(),
    };
    return { note: notesStore[index] };
  },

  deleteDocument: (payload: unknown) => {
    const { id } = (payload as { id?: string }) || {};
  ...
  getAppSettings: () => ({
    settings: { currency: "EUR", taxRate: 0.21, paymentTerms: [{ code: "NET30", days: 30 }] },
    source: "parameter",
  }),

  getOptionalAppSettings: () => ({
    settings: { currency: "USD", taxRate: 0, paymentTerms: [{ code: "NET30", days: 30 }] },
    source: "defaults",
  }),
  ...
  sendNotification: (payload: unknown) => {
    ...
    return { messageId: `mock-message-${Date.now()}`, status: "SENT", subject, body };
  },

  sendBulkNotification: (payload: unknown) => {
  ...
  listUsers: () => ({
    users: [
      { id: "mock-user-id", firstName: "Local", lastName: "Developer", email: "dev@localhost" },
      { id: "mock-user-2", firstName: "Alice", lastName: "Johnson", email: "alice@example.com" },
    ],
    count: 2,
  }),
  ...
  getJobStatus: (payload: unknown) => {
    const { jobId } = (payload as { jobId?: string }) || {};
    return {
      jobId: jobId || "mock-job",
      functionName: "_processInBackground",
      status: "COMPLETED",
